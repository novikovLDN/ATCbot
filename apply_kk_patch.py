#!/usr/bin/env python3
"""Apply translation_patch_kk.json to app/i18n/kk.py."""
...
PATCH_FILE = ROOT / "translation_patch_kk.json"
KK_FILE = ROOT / "app" / "i18n" / "kk.py"


def main() -> int:
    patch = json.loads(PATCH_FILE.read_text(encoding="utf-8"))
    content = KK_FILE.read_text(encoding="utf-8")
    ...
    lines = ["# -*- coding: utf-8 -*-", '"""kk strings."""', "", "LANG = {"]
    ...
    try:
        exec(KK_FILE.read_text(encoding="utf-8"), {})
    except SyntaxError as e:
        print(f"ERROR: Syntax error: {e}", file=sys.stderr)
        return 1

    print(f"Applied {applied} translations to {KK_FILE}")
