...
async def _notify_user_declined(
    ...
    text = (
        ...
        f"Заказ <code>{order_id[-12:]}</code> не был подтверждён банком. "
        f"Подписка/товар не активирован.\n\n"
        f"🎫 <b>Код обращения:</b> <code>{ticket}</code>\n\n"
        f"<b>Если деньги были списаны</b> — сохраните этот код и напишите "
        f"в поддержку, мы вернём средства.\n\n"
        f"Если списания не было — можно попробовать оплатить ещё раз."
    )
