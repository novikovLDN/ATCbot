#!/usr/bin/env python3
"""Apply translation_patch_ar.json to app/i18n/ar.py."""
...
PATCH_FILE = ROOT / "translation_patch_ar.json"
AR_FILE = ROOT / "app" / "i18n" / "ar.py"
...
def main() -> int:
    ...
    lines = ["# -*- coding: utf-8 -*-", '"""ar strings."""', "", "LANG = {"]
    ...
    try:
        exec(AR_FILE.read_text(encoding="utf-8"), {})
    except SyntaxError as e:
        print(f"ERROR: Syntax error: {e}", file=sys.stderr)
        return 1

    print(f"Applied {applied} translations to {AR_FILE}")
