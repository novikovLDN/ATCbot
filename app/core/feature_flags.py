...
def background_workers_paused(worker_name: str) -> bool:
    """Выключен ли kill switch фоновых воркеров.

    ЗАЧЕМ ОБЩИЙ ХЕЛПЕР
        FEATURE_BACKGROUND_WORKERS_ENABLED — аварийный рубильник: им
        останавливают фоновую активность, когда с ботом что-то не так.
        Проверяли его только четыре воркера из девяти, и остальные пять
        (напоминания, ферма, монитор трафика, синхронизация с сайтом,
        отложенные рассылки) продолжали работать как ни в чём не бывало.
        Рубильник, который останавливает меньше половины, хуже отсутствия
        рубильника: им пользуются, считая, что фон встал.

        Проверка через одну функцию делает поведение одинаковым и даёт
        единый маркер в логах.

    ПОЧЕМУ ПРОВЕРЯТЬ ВНУТРИ ЦИКЛА, А НЕ ПРИ ЗАПУСКЕ
        Флаг читается из окружения и может измениться без перезапуска
        процесса. Проверка на старте остановила бы воркер только при
        следующем деплое — то есть в момент, когда рубильник уже не нужен.

    Returns:
        True — итерацию выполнять не нужно.
    """
    if get_feature_flags().background_workers_enabled:
        return False
    logger.warning(
        "[FEATURE_FLAG] Фоновые воркеры выключены "
        "(FEATURE_BACKGROUND_WORKERS_ENABLED=false), пропускаем итерацию: %s",
        worker_name,
    )
    return True
