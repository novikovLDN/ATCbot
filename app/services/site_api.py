...
API endpoints:
    ...
    POST /api/bot/unlink                   — unlink telegram_id from account
"""
...
async def unlink_account(telegram_id: int) -> Optional[Dict[str, Any]]:
    """
    Unlink telegram_id from site account.
    ...
    """
    result = await _request("POST", "/api/bot/unlink", json={
        "telegramId": str(telegram_id),
    })
    if result is not None:
        invalidate_status_cache(telegram_id)
