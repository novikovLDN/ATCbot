...
async def get_user_happ_crypto_link(uuid: str) -> Optional[str]:
    """Return the Remnawave-generated Happ Crypto deeplink for the user.

    Remnawave panel exposes a "Happ Crypto Link" — an encrypted Happ import
    URL of the form ``happ://crypto/<base64_blob>`` that ships configs
    without exposing the panel hostname.

    Field naming has changed across Remnawave versions, so we probe several
    known shapes and log the user-object keys when nothing is found, so we
    can adapt if the panel exposes the link under another name.
    """
    user = await get_user(uuid)
    if not user:
        return None

    # 1) Newer Remnawave: nested happ object
    happ_obj = user.get("happ") or {}
    if isinstance(happ_obj, dict):
        for key in ("cryptoLink", "crypto_link", "subscriptionCryptoLink"):
            val = happ_obj.get(key)
            if isinstance(val, str) and val.startswith("happ://"):
                return val

    # 2) Older / alternate flat fields
    for key in (
        "happCryptoLink",
        "happ_crypto_link",
    ...
    # 3) Some panels return a list of subscription links with a type tag
    links = user.get("subscriptionLinks") or user.get("subscription_links") or []
    if isinstance(links, list):
        for link in links:
            if not isinstance(link, dict):
                continue
            link_type = (link.get("type") or link.get("client") or "").lower()
            url = link.get("url") or link.get("link") or ""
            if (
                isinstance(url, str)
                and url.startswith("happ://crypto/")
                and ("crypto" in link_type or "happ" in link_type or not link_type)
            ):
                return url

    # 4) Not found — log what's actually in the response (one-time-ish)
    try:
        keys = sorted(user.keys()) if isinstance(user, dict) else []
        happ_keys = sorted(happ_obj.keys()) if isinstance(happ_obj, dict) else []
        logger.warning(
            "REMNAWAVE_CRYPTO_LINK_NOT_FOUND uuid=%s top_keys=%s happ_keys=%s",
            uuid[:8] if uuid else "?", keys, happ_keys,
        )
    except Exception:
        pass
    return None
