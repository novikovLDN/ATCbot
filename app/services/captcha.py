...
def render_prompt_text(_challenge: Challenge) -> str:
    """Caption под фото капчи. Имя цели не указываем — оно уже подписано
    на самом изображении («Медведь», «Кролик» и т.п.), задача юзера —
    ткнуть соответствующую кнопку."""
    return (
        "🤖 Пожалуйста, выберите из списка ниже, чтобы подтвердить, "
        "что вы не робот."
    )
