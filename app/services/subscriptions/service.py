...
async def disable_if_expired(telegram_id: int) -> bool:
    """Отключить подписку, если срок уже вышел. Возвращает True, если отключили.

    Зачем обёртка. Обработчики (экран профиля, главное меню) звали
    database.check_and_disable_expired_subscription напрямую — это
    единственное место, где UI-слой лез в базу мимо сервиса. Смешанное
    обращение к слоям стоит дорого не само по себе, а потому, что мимо
    сервиса проходят и его гарантии: здесь мы гасим исключение.

    Почему гасим. Функция вызывается ради побочного эффекта прямо перед
    отрисовкой экрана. Внутри неё сетевой вызов в Remnawave (удаление
    пользователя), и он может отвалиться по таймауту. Раньше такое падение
    роняло весь обработчик профиля: человек нажимал «Профиль» и не получал
    ничего. Просроченная подписка подождёт до следующего клика или до
    сторожевого воркера, а экран пользователь должен увидеть всегда.
    """
    try:
        return await database.check_and_disable_expired_subscription(telegram_id)
    except Exception as e:
        logger.error(
            "EXPIRY_CHECK_FAILED telegram_id=%s error=%s — экран показываем как есть",
            telegram_id, e,
        )
        return False
