...
ПОРЯДОК ПОДКЛЮЧЕНИЯ — ЧАСТЬ ПОВЕДЕНИЯ
    ...
    По этой же причине `scheduled` подключён ПЕРВЫМ, а не последним, как
    было в исходном файле. Там `GET /scheduled` стоял ниже
    `GET /{broadcast_id}` и был недостижим: список отложенных рассылок
    отвечал 422 «broadcast_id не число», экран не открывался. Дефект
    приехал из исходного файла, разбивка его не создавала.

    Остальной порядок повторяет исходный: read → uploads → send. Менять
    его без нужды нельзя, а если менять — смотреть на весь список
    маршрутов целиком, а не на один добавляемый. За порядком следит
    test_broadcasts_route_split: любой литеральный путь обязан быть
    объявлен раньше перехватывающего `/{broadcast_id}`.
...
# Первым — иначе `GET /scheduled` перехватывает `GET /{broadcast_id}`.
...
# которого пустые и префикс, и путь маршрута. Пустой путь ни с чем не
# пересекается, поэтому место в списке на поведение не влияет.
