...
FRESH_TTL = 0             # 0 → fresh-кеша НЕТ: КАЖДЫЙ запрос (ручное обновление
                          # ИЛИ авто-опрос — на сервере они неотличимы) рефетчит
                          # СВЕЖЕЕ из панели. Гарантирует: юзер нажал «обновить» →
                          # сразу получил актуальный список серверов, без ожидания
                          # TTL и без /aggflush. Нагрузка ок: токен персональный,
                          # опрашивается раз/час вразнобой; параллельные запросы на
                          # 1 token схлопывает singleflight. Stale-tier (24ч) ниже
                          # НЕ отключается — при падении панели отдаём последнюю
                          # копию вместо 503 (см. шаг 6 в GET /a/{token}).
                          # История: 2ч → второй опрос отдавал старьё; 60с → до
                          # минуты старьё; 0 → всегда свежее (то что просили).
...
def _cache_set(token: str, body: bytes, headers: dict) -> None:
    """Обычный ответ: fresh 0s (кеша нет) + stale 24h (fallback при падении панели)."""
...
async def aggregate(
    ...
    Multi-tier caching:
      1. FRESH_TTL=0    → fresh-кеша нет, каждый запрос = свежий fetch
