...
FRESH_TTL = 60            # 60с — КОРОЧЕ интервала опроса клиентов (1ч), чтобы
                          # каждый часовой refresh промахивался мимо кеша и тянул
                          # СВЕЖЕЕ из панели. Так любое изменение (в панели ИЛИ в
                          # боте) доходит до юзера на его следующем опросе без
                          # ручного /aggflush. 60с нужны только чтобы схлопнуть
                          # одновременные опросы с нескольких устройств и ретраи
                          # (singleflight и так это делает). Токен персональный,
                          # опрашивается раз/час → нагрузка на панель мизерная.
                          # Раньше стоял 2ч: экономил копейки (юзер заходит редко),
                          # но второй опрос через час отдавал СТАРЫЙ список серверов.
...
def _cache_set(token: str, body: bytes, headers: dict) -> None:
    """Обычный ответ: fresh 60s + stale 24h."""
...
async def aggregate(
    ...
    Multi-tier caching:
      1. FRESH_TTL=60s  → hit → мгновенно (окно только для схлопа бурстов)
