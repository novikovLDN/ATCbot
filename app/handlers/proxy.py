...
def _sales_text() -> str:
    return (
        ...
        "Быстрый способ вернуть Telegram там, где он замедлен или "
        "ограничен — без установки приложений и сложных настроек.\n\n"
        "• Подключение в одно касание\n"
        "• Не расходует трафик и не влияет на скорость\n"
        "• Покупка разовая — ссылка остаётся у вас навсегда\n\n"
        "⚠️ <b>Важно:</b> прокси и VPN могут конфликтовать. Если на "
        "устройстве уже работает наш VPN — прокси не нужен. Используйте "
        "прокси на устройствах, где Telegram работает <b>без VPN</b>.\n\n"
        "💡 Прокси ускоряет <b>только Telegram</b>. Чтобы открыть сайты, "
        "сервисы и приложения целиком — нужен VPN Atlas Secure.\n\n"
        f"Стоимость: <b>{config.PROXY_PRICE_RUBLES} ₽</b> разово.\n"
