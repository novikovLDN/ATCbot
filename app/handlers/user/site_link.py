...
# =========================================================================
# D) Unlink site account (double confirmation)
# =========================================================================

@site_link_router.callback_query(F.data == "unlink_site")
async def callback_unlink_site_step1(callback: CallbackQuery, state: FSMContext):
    """Step 1: Ask user to confirm unlinking."""
    if not await ensure_db_ready_callback(callback):
        return
    ...
    if not site_user_id:
        await callback.message.edit_text(
            i18n_get_text(language, "site_link.not_linked"),
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=i18n_get_text(language, "site_link.unlink_confirm_yes"),
            callback_data="unlink_site_confirm",
        )],
        [InlineKeyboardButton(
            text=i18n_get_text(language, "common.back"),
            callback_data="menu_settings",
    ...
    await callback.message.edit_text(
        i18n_get_text(language, "site_link.unlink_step1"),
        reply_markup=keyboard,
    )


@site_link_router.callback_query(F.data == "unlink_site_confirm")
async def callback_unlink_site_step2(callback: CallbackQuery, state: FSMContext):
    """Step 2: Final confirmation — are you sure?"""
    if not await ensure_db_ready_callback(callback):
        return
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=i18n_get_text(language, "site_link.unlink_final_yes"),
            callback_data="unlink_site_execute",
        )],
        [InlineKeyboardButton(
            text=i18n_get_text(language, "common.back"),
            callback_data="menu_settings",
    ...
    await callback.message.edit_text(
        i18n_get_text(language, "site_link.unlink_step2"),
        reply_markup=keyboard,
    )


@site_link_router.callback_query(F.data == "unlink_site_execute")
async def callback_unlink_site_execute(callback: CallbackQuery, state: FSMContext):
    """Execute the unlink after double confirmation."""
    if not await ensure_db_ready_callback(callback):
        return
    ...
    # Call site API to unlink
    try:
        result = await site_api.unlink_account(telegram_id)
        logger.info("SITE_UNLINK: API result for user %s: %s", telegram_id, result)
    except Exception as e:
        logger.exception("SITE_UNLINK: API error for user %s: %s", telegram_id, e)
        result = None

    # Clear local site_user_id regardless of API result
    # (if API fails, user can re-link later)
    await database.clear_site_user_id(telegram_id)
    site_api.invalidate_status_cache(telegram_id)

    logger.info("SITE_UNLINK: user %s unlinked from site (site_user_id was %s)", telegram_id, site_user_id)

    await callback.message.edit_text(
        i18n_get_text(language, "site_link.unlink_done"),
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                text=i18n_get_text(language, "common.back"),
                callback_data="menu_settings",
