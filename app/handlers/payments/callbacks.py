...
async def callback_tariff_type(callback: CallbackQuery, state: FSMContext):
    ...
    # Кнопка назад: для бизнес-тарифов → каталог клиентских, для обычных → главный экран тарифов
...
async def callback_corporate_access_request(callback: CallbackQuery, state: FSMContext):
    ...
    Shows business client tariffs (by daily client limit).
    """
    try:
        await callback.answer()
    except Exception:
    ...
    buttons = []
    for tariff_key, tariff_info in config.BIZ_CLIENT_TARIFFS.items():
        label = tariff_info.get("label", tariff_key)
        price_30 = tariff_info.get(30, {}).get("price", 0)
        btn_text = f"📦 {label} · от {price_30:,} ₽/мес".replace(",", " ")
        buttons.append([InlineKeyboardButton(text=btn_text, callback_data=f"tariff:{tariff_key}")])
    buttons.append([InlineKeyboardButton(text=i18n_get_text(language, "common.back"), callback_data="menu_buy_vpn")])
