...
# ====================================================================================
# STEP 4F: Международная оплата (Platega international)
# ====================================================================================

@gift_router.callback_query(F.data == "gift_pay:international", GiftState.choose_payment_method)
async def callback_gift_pay_international(callback: CallbackQuery, state: FSMContext):
    """Оплата подарка через международный эквайринг."""
    telegram_id = callback.from_user.id
    ...
    try:
        price_rubles = price_kopecks / 100.0
        purchase_id = await database.create_pending_purchase(
            telegram_id=telegram_id, tariff=tariff, period_days=period_days,
            price_kopecks=price_kopecks, purchase_type="gift",
        )
        await state.update_data(gift_purchase_id=purchase_id)

        tx_data = await platega_service.create_transaction(
            amount_rubles=price_rubles,
            description=f"Подарок Atlas Secure — {tariff} {period_days}d",
            purchase_id=purchase_id,
        ...
        logger.exception(f"Error creating gift international payment: {e}")
        await callback.answer(i18n_get_text(language, "errors.payment_create"), show_alert=True)
        await state.clear()
