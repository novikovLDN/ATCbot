"""Оплата картой через Lava.

ЧТО ЗДЕСЬ
    Один экран: покупка, счёт в Lava, кнопка со ссылкой на оплату.

ПОЧЕМУ ВЫДЕЛЕНО
    Ещё один внешний провайдер со своим API и своим форматом ответа
    (invoice_id / payment_url). К Platega и CryptoBot отношения не имеет.

ЧТО ЛЕГКО СЛОМАТЬ
    В отличие от CryptoBot, здесь сообщение со ссылкой удаляется по
    таймауту (_schedule_invoice_deletion): счёт Lava протухает, и человек,
    нажавший старую кнопку, получил бы ошибку провайдера вместо
    объяснения. Убрать задачу удаления — вернуть эту ошибку.
"""
import asyncio
import logging
