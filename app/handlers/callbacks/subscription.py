"""Обработчики экранов подписки: автопродление, триал, профиль, продление.

Что здесь НЕ живёт и почему: экраны «VIP-доступ» (menu_vip_access) и
«История подписок» (subscription_history) удалены — на них не вело ни одной
кнопки во всём боте. Тексты в словарях (main.vip_access_text,
subscription.history) оставлены: если экраны понадобятся, вернуть их дешевле,
чем переписывать тексты на семь языков заново.
"""
import asyncio
import logging
import time
