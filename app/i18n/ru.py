...
LANG = {
    ...
    # Общая инструкция для всех платформ — universal raw subscription
    # URL работает в любом Xray-совместимом клиенте (Happ / Incy /
    # V2rayTun / v2rayN / …).  Отдельные тексты per-platform сохранены,
    # чтобы иметь свободу их развести обратно без правки кода.
    'setup.connect_ios': (
        "📋 <b>Ручная установка</b>\n\n"
        "1. Скопируйте ссылку подписки ниже — тап по ссылке скопирует её в буфер.\n"
        "2. Откройте любой VPN-клиент из списка (Happ / Incy / V2rayTun и т.п.).\n"
        "3. Найдите пункт <b>«Добавить подписку»</b> / <b>Import subscription</b>.\n"
        "4. Вставьте ссылку из буфера.\n"
        "5. Клиент загрузит серверы автоматически.\n\n"
        "💡 <i>Совет:</i> если ссылка не сработала — попробуйте кнопку «Добавить в …» "
        "на предыдущем экране, она откроет клиент напрямую."
    ),
    ...
    'setup.connect_windows': (
        "📋 <b>Ручная установка</b>\n\n"
        "1. Скопируйте ссылку подписки ниже — тап по ссылке скопирует её в буфер.\n"
        "2. Откройте любой Xray-совместимый VPN-клиент (Happ, v2rayN и т.п.).\n"
        "3. Найдите пункт <b>«Добавить подписку»</b> / <b>Import subscription</b>.\n"
        "4. Вставьте ссылку из буфера.\n"
        "5. Клиент загрузит серверы автоматически.\n\n"
        "💡 <i>Совет:</i> если ссылка не сработала — попробуйте кнопку «Добавить в …» "
    ...
    # Единый label — одна ссылка на клиент, без разделения Happ / Incy.
    # Ключи *_incy_label оставлены пустыми на случай, если код где-то
    # ещё их читает — удалять из словаря нельзя, i18n.get_text вернёт
    # KeyError.  См. app/i18n/en.py — там та же схема.
    'setup.key_vpn_label': "🔑 <b>Ссылка подписки</b> (обычные безлимитные сервера):",
    'setup.key_bypass_label': "🔑 <b>Ссылка подписки — обход</b> (белые списки РФ):",
    'setup.key_vpn_incy_label': "🔑 <b>Ссылка подписки</b> (обычные безлимитные сервера):",
    'setup.key_bypass_incy_label': "🔑 <b>Ссылка подписки — обход</b> (белые списки РФ):",
