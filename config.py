...
# Комбо-тарифы: подписка + пакет ГБ обхода одной покупкой.
# ВАЖНО: «Комбо Плюс» и «Плюс» — РАЗНЫЕ тарифы, а не варианты одного.
# У них разная цена, разное наполнение и разные периоды. Совпадает только
# уровень VPN-доступа, поэтому при продлении комбо нельзя понижать до
# одноимённой обычной подписки. Цены и ГБ живут в COMBO_TARIFFS ниже.
COMBO_TARIFF_TYPES = ("combo_basic", "combo_plus")

# Все допустимые типы подписок (для валидации в БД и хендлерах).
# Комбо сюда НЕ входит: в колонке subscription_type исторически хранится
# базовый уровень доступа, а признак комбо лежит отдельным флагом is_combo.
VALID_SUBSCRIPTION_TYPES = ("basic", "plus") + BIZ_TARIFFS

# Типы подписок, которые считаются ПЛАТНЫМИ при подсчёте «сколько людей
# сейчас платит». Комбо здесь обязано быть: в колонке subscription_type у
# него два представления — историческое 'plus' + is_combo=TRUE и явное
# 'combo_basic' / 'combo_plus' (так его кладёт выдача админом, так его ищет
# сегментация рассылок). Первое попадает в счёт через 'plus', второе — только
# если перечислить его отдельно.
#
# ЗАЧЕМ ОТДЕЛЬНОЕ ИМЯ, А НЕ VALID_SUBSCRIPTION_TYPES
#     Тот список — про валидацию записи в базу, и комбо там нет намеренно.
#     Использовать его для подсчёта означало бы терять комбо-подписчиков.
#     Ровно это и происходило в database/analytics_stats.py, где список был
#     выписан руками: активных подписок показывалось меньше, чем есть, и
#     заметить это по числу невозможно — оно просто другое.
PAID_SUBSCRIPTION_TYPES = VALID_SUBSCRIPTION_TYPES + COMBO_TARIFF_TYPES

# Тарифы, которые админ может выдать вручную — из бота или из дашборда.
# Здесь комбо присутствует: администратор должен уметь выдать «Комбо Плюс»
# так же, как обычную подписку. Раньше этого списка не было, и выдать
# комбо через дашборд было нельзя вовсе.
GRANTABLE_TARIFF_TYPES = VALID_SUBSCRIPTION_TYPES + COMBO_TARIFF_TYPES

# ── Типы покупок ──────────────────────────────────────────────────────
#
# Покупка в боте — это не только подписка. Часть товаров не продлевает
# доступ, а выдаётся отдельно: ключ Steam, аккаунт Spotify, Apple ID,
# пакет гигабайтов, подарок, эффект на ферме, пополнение баланса.
#
# У них у всех period_days = 0, и раньше это использовали как признак:
# «нет срока — значит пополнение баланса». Список исключений при этом
# лежал в трёх местах и в одном из них был неполон — steam и proxy в нём
# отсутствовали. Держалось всё на том, что до опасной ветки эти типы
# перехватывались раньше. Любой новый путь оплаты, зовущий
# finalize_purchase напрямую, зачислил бы оплату Steam на внутренний
# баланс, а Steam бы не пополнился — товар при этом выглядел оплаченным.
#
# Поэтому: один список, и никакого угадывания по period_days.
NON_SUBSCRIPTION_PURCHASE_TYPES = (
    "balance_topup",      # пополнение внутреннего баланса
    "traffic_pack",       # пакет ГБ обхода
    "gift",               # подарочная подписка (выдаётся получателю)
    "apple_id",           # аккаунт Apple ID
    "steam",              # пополнение Steam
    "spotify",            # подписка Spotify
    "proxy",              # MTProto-прокси
    "telegram_premium",   # Telegram Premium
    "telegram_stars",     # звёзды Telegram
    "farm_effect",        # эффект в мини-игре
)


def is_subscription_purchase(purchase_type: str) -> bool:
    """Продлевает ли эта покупка VPN-подписку.

    Неизвестный тип считаем подпиской: так новая покупка попадёт в
    штатный путь выдачи и упадёт заметно, а не зачислится молча на
    баланс.
    """
    return (purchase_type or "subscription") not in NON_SUBSCRIPTION_PURCHASE_TYPES


# ── Допуск на расхождение суммы платежа ───────────────────────────────
#
# Провайдер может прислать сумму, отличную от ожидаемой на копейки:
# округление комиссии, конвертация валюты, свои правила округления.
# Отвергать такой платёж нельзя — человек заплатил.
#
# Допуск процентный, а не фиксированный: 0.5% от суммы с нижней границей
# в 50 копеек. Для 149 ₽ это 75 копеек, для 1199 ₽ — 6 ₽.
#
# Порог должен быть ОДИН. Их было три: ±1 ₽ в сервисном слое, эта формула
# в слое БД и ещё раз ±1 ₽ в модулях провайдеров — причём там при
# превышении платёж не отвергался, а подставлялась пришедшая сумма. Из-за
# расхождения существовала зона, где платёж проходил одну проверку и
# падал на второй: покупка на 199 ₽ с расхождением в 1 ₽ ровно.
PAYMENT_AMOUNT_TOLERANCE_PERCENT = 0.005
PAYMENT_AMOUNT_TOLERANCE_MIN_RUBLES = 0.50


def payment_amount_tolerance(expected_amount_rubles: float) -> float:
    """Насколько присланная сумма может отличаться от ожидаемой."""
    return max(
        PAYMENT_AMOUNT_TOLERANCE_MIN_RUBLES,
        abs(expected_amount_rubles) * PAYMENT_AMOUNT_TOLERANCE_PERCENT,
    )
