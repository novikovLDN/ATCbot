...
def _migrated_row(sub_url: str = PANEL_SUB_URL):
    return {
        "telegram_id": 42,
...
def test_legacy_sub_redirects_migrated_user_to_remnawave():
    with patch.object(subscription_proxy, "remnawave_api") as api_mock, \
        ...
        api_mock.get_user = AsyncMock(return_value={"subscriptionUrl": PANEL_SUB_URL})
...
def test_legacy_sub_returns_404_when_panel_lookup_returns_no_url():
    """Migrated row exists but panel returned nothing → still try samopis fallback."""
    with patch.object(subscription_proxy, "remnawave_api") as api_mock, \
         ...
         patch("database.get_subscription_by_samopis_uuid",
               new=AsyncMock(return_value=None)):
        # Panel returns None for the entity
