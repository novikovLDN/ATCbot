...
class TestIsTrialExpired:
    ...
    async def test_trial_not_expired(self):
        ...
        assert await is_trial_expired(12345, future, now) is False
...
class TestCalculateTrialTiming:
    ...
    def test_calculate_timing_after_expiry(self):
        """Should handle timing after expiry (clamped to 0)"""
        expiry = datetime(2024, 1, 18, 12, 0, 0)
        now = datetime(2024, 1, 20, 12, 0, 0)  # 2 days after expiry

        result = calculate_trial_timing(expiry, now)

        # hours_until_expiry is clamped to 0 by max(0.0, ...)
        assert result["hours_until_expiry"] == 0
        # hours_since_activation = 72 - (-48) = 120, but clamped by max(0.0, ...)
        assert result["hours_since_activation"] == 120
...
class TestShouldSendNotification:
    ...
    async def test_should_send_at_6h_mark(self):
        ...
        expiry = datetime(2024, 1, 18, 12, 0, 0)
        now = datetime(2024, 1, 15, 18, 0, 0)  # 6h after activation (66h before expiry)
        schedule_entry = {"hours": 6, "key": "trial.notification_6h", "has_button": False, "db_flag": "trial_notif_6h_sent"}
        flags = {"trial_notif_6h_sent": False}

        with patch('app.services.trials.service.database') as mock_db:
            mock_db._to_db_utc = lambda dt: dt
            mock_conn = AsyncMock()
            mock_conn.fetchrow = AsyncMock(return_value=None)  # no paid subscription
            result = await should_send_notification(
                12345, expiry, expiry, schedule_entry, flags, now, mock_conn
            )
    ...
    async def test_should_not_send_if_already_sent(self):
        ...
        schedule_entry = {"hours": 6, "key": "trial.notification_6h", "has_button": False, "db_flag": "trial_notif_6h_sent"}
        flags = {"trial_notif_6h_sent": True}
...
class TestShouldSendFinalReminder:
    ...
    async def test_should_send_6h_before_expiry(self):
        ...
        with patch('app.services.trials.service.database') as mock_db:
            ...
            result = await should_send_final_reminder(
                12345, expiry, expiry, False, now, mock_conn
            )
            assert result[0] is True
    ...
    async def test_should_not_send_if_already_sent(self):
        ...
        with patch('app.services.trials.service.database') as mock_db:
            ...
            mock_conn.fetchrow = AsyncMock(return_value=None)
            result = await should_send_final_reminder(
                12345, expiry, expiry, True, now, mock_conn
            )
            assert result[0] is False
