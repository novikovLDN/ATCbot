-- Migration 029: Add farm game columns to users table
-- Purpose: Add farm_plots JSONB and farm_plot_count INTEGER columns for farm game
...
-- Add farm_plots column if it doesn't exist (idempotent)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Add farm_plot_count column if it doesn't exist (idempotent)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
