...
- Находок: **218** — P0 13, P1 60, P2 108, P3 37
- Прошли состязательную перепроверку: **21**
- Опровергнуто скептиками: **6**
- Не перепроверялись: **197** — гипотезы, а не факты

Перепроверка охватывала три самые тяжёлые находки каждого домена. Всё остальное требует подтверждения перед тем, как по нему что-то менять.
...
5. ✅ **Оплата Spotify картой через Telegram Payments уходит в ветку VPN-подписки — деньги списаны, товар не выдан**  
   `app/handlers/payments/payments_messages.py:843` · домен payments
6. ✅ **Удаление xray-веток вместе с XRAY_API_* переменными глушит весь провижининг**  
   `config.py:352` · домен vpn
7. ✅ **Авто-починка CHECK-констрейнтов в create_pending_purchase сужает список типов и ломает Steam/прокси/ферму навсегда**  
   `database/subscriptions.py:4180` · домен payments
8. ✅ **Вебхук Lava не проверяет подпись вообще — бесплатная подписка любому, кто знает purchase_id**  
   `lava_service.py:195` · домен payments
9. ⚠️ **Ветка «Оплата с баланса» для покупки подписки допускает двойное списание: проверка идемпотентности мертва**  
   `app/handlers/callbacks/payments_callbacks.py:449` · домен payments
10. ⚠️ **Гонка при сборе урожая: двойной клик начисляет награду несколько раз**  
   `app/handlers/game.py:1002` · домен games
11. ⚠️ **Шторм превращает любое растение в 50 % награды независимо от прогресса — доминирующая стратегия фермы**  
   `app/handlers/game.py:1523` · домен games
12. ⚠️ **Несовпадение суммы в вебхуке отдаёт провайдеру HTTP 200 «already_processed» — платёж молча теряется без алерта**  
   `app/services/payments/confirmation.py:177` · домен payments
13. ⚠️ **Оффлайн-режим шторма выгоднее онлайна: авто-сбор 50 % без единого действия, петля «зашёл раз в цикл»**  
...
- [Платежи и провайдеры](payments.md) — 25
