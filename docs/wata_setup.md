...
### 2a. Настройка комиссии эквайринга

По умолчанию Wata показывает юзеру сумму + комиссию (~2%). Т.е. если
мы просим 199 ₽, юзер видит и платит 203.06 ₽. Webhook приходит с 203.06,
а наш pending — на 199. Раньше это ломало `finalize_purchase` из-за
mismatch. Сейчас у нас tolerance 3% — покрывает.

**Правильнее** — в кабинете Wata переключить настройку эквайринга:
`Кабинет → Настройки → Комиссия → "Мерчант несёт комиссию"`
(если такая опция есть). Тогда юзер платит ровно нашу сумму, комиссия
списывается из наших поступлений. Плюсы: honest pricing, tolerance
можно вернуть к 0.5% для лучшей защиты от подмены.
