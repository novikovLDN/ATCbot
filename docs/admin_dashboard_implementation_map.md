...
| `GET /api/premium-repair/status` | `premium_repair_job.get_status()` — «Премиум больше 5 лет»: `check` (проверка: summary, would_fix, eta) и `apply` (state / total / done / fixed / errors / skipped / remaining / limit), app_settings `premium_repair_job`; `running` без живой задачи = `interrupted` |
| `GET /api/premium-repair/report` · `/report.csv` | строки последней проверки / починки (app_settings `premium_repair_report`): страница JSON (`offset`, `limit` ≤ 10000) или CSV с колонками CLI-отчёта; скачивание CSV — audit_log `premium_repair_report_csv` |
| `POST /api/premium-repair/check` · `/start` · `/resume` · `/pause` · `/stop` | `premium_repair_job.*` — одна фоновая задача за раз: проверка (`premium_repair.build_plan` + `summarize`, ничего не пишет) или починка (`premium_repair.apply_plan`: PATCH `{id, expireAt}` premium-сущности ≤ 2/с, свежее чтение БД перед каждым, ошибки считаются и не прерывают); `/start` принимает `{limit}` — пробный запуск первых N, затем пауза; «Продолжить» заново сканирует панель. Idempotency-Key, audit_log `premium_repair_check_start` / `premium_repair_apply_*`, по завершении — один алерт админу. CLI `scripts/fix_premium_over_issuance.py --apply` при незавершённой задаче отказывает без `--force`. Экран: «Ещё» → «Настройки» → «Премиум больше 5 лет» |
